...
function StatusBadge({ children }) {
  return <span className={`status-badge status-${String(children).toLowerCase()}`}>{children}</span>
}
...
function AdminDashboard({ setPage }) {
  ...
  const stats = useMemo(() => ({
    ...
    messages: initialConversations.reduce((sum, item) => sum + item.messages, 0),
  ...
  const renderDashboard = () => (
    <>
      ...
      <div className="admin-stats-grid">
        <StatCard icon="👥" label="Total Users" value={stats.users} detail="Registered users" />
        <StatCard icon="💬" label="Conversations" value={stats.conversations} detail="All conversations" />
        <StatCard icon="✉️" label="Messages" value={stats.messages} detail="User + AI messages" />
        <StatCard icon="📄" label="Documents" value={stats.documents} detail="Uploaded documents" />
        <StatCard icon="★" label="Feedback" value={stats.feedback} detail="User responses" />
      </div>
      ...
      <div className="admin-panels-grid">
        <section className="admin-panel">
          <div className="panel-heading">
            ...
            <button onClick={() => selectSection("users")}>View all</button>
          </div>
        ...
        <section className="admin-panel">
          <div className="panel-heading">
            ...
            <button onClick={() => selectSection("feedback")}>View all</button>
          </div>
  ...
  const renderUsers = () => (
    <>
      <div className="admin-page-heading">
        ...
        <div className="admin-count">{users.length} users</div>
      </div>
      <div className="admin-table-panel">
        ...
        <div className="table-scroll">
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id}>
                  ...
                  <td>{user.joined}</td>
                  <td>
                    <button className="table-action" onClick={() => handleToggleUser(user.id)}>
          ...
          {filteredUsers.length === 0 && <div className="table-empty">No users found.</div>}
        </div>
      </div>
    </>
  )
  ...
  const renderConversations = () => (
    <>
      <div className="admin-page-heading"><div><h1>Conversations</h1><p>Monitor conversations created by users.</p></div></div>
      <div className="admin-table-panel">
        <div className="table-scroll">
          <table>
            <thead><tr><th>ID</th><th>User</th><th>Title</th><th>Messages</th><th>Status</th><th>Last Activity</th></tr></thead>
            <tbody>{initialConversations.map((item) => (
              <tr key={item.id}><td>#{item.id}</td><td><strong>{item.user}</strong></td><td>{item.title}</td><td>{item.messages}</td><td><StatusBadge>{item.status}</StatusBadge></td><td>{item.date}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </div>
  ...
  const renderDocuments = () => (
    <>
      <div className="admin-page-heading"><div><h1>Documents</h1><p>Monitor uploaded documents and processing status.</p></div></div>
      <div className="admin-table-panel">
        <div className="table-scroll">
          <table>
            <thead><tr><th>File</th><th>User</th><th>Type</th><th>Size</th><th>Status</th><th>Uploaded</th></tr></thead>
            <tbody>{initialDocuments.map((item) => (
              <tr key={item.id}><td><strong>📄 {item.filename}</strong></td><td>{item.user}</td><td>{item.type}</td><td>{item.size}</td><td><StatusBadge>{item.status}</StatusBadge></td><td>{item.date}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </div>
  ...
  const renderFeedback = () => (
    <>
      <div className="admin-page-heading"><div><h1>Feedback</h1><p>Review ratings and comments from users.</p></div></div>
      <div className="admin-table-panel">
        <div className="table-scroll">
          <table>
            <thead><tr><th>User</th><th>Rating</th><th>Type</th><th>Comment</th><th>Date</th></tr></thead>
            <tbody>{initialFeedback.map((item) => (
              <tr key={item.id}><td><strong>{item.user}</strong></td><td><span className="stars">{item.rating}/5</span></td><td><StatusBadge>{item.type}</StatusBadge></td><td>{item.text}</td><td>{item.date}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </div>
    ...
    <div className="admin-layout">
      <aside className={`admin-sidebar ${sidebarOpen ? "admin-sidebar-open" : ""}`}>
        <div className="admin-brand">
          ...
          <div><strong>ChatBot</strong><span>Administration</span></div>
        </div>
        ...
        <div className="admin-sidebar-bottom">
          ...
          <button className="admin-logout-sidebar" onClick={handleLogout}>↪ Logout</button>
        </div>
      </aside>
      ...
      <div className="admin-main">
        <header className="admin-topbar">
          ...
          <div className="topbar-right"><span className="admin-online-dot"></span><span>Administrator</span><button onClick={handleLogout}>Logout</button></div>
        </header>
